...
```
    ...
    Telegram `/new` archives the current conversation as a normal Gateway
    session row and starts a separately addressable child session. The session
    remains visible in the Control UI archive instead of becoming only a reset
    transcript artifact.

    - `/session` lists the current conversation's sessions, 10 per page. The
      stable five-character identifiers are rendered as inline code so Telegram
      clients can copy them.
    - `/session page <number>` moves through the list. Only Previous and Next
      use inline buttons.
    - `/session <id>` archives the current row, restores the selected row, and
      moves the Telegram conversation binding to it.
    - `/change <title>` is an alias for `/name <title>`. A manual title wins over
      the default title derived from the session's first user message.
    - `/session idle` and `/session max-age` keep their existing focused-session
      lifecycle behavior.
