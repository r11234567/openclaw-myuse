...
Plugin-owned session binding adapters can set
`capabilities.archiveCurrentSessionOnNew` when `/new` must preserve the current
Gateway session row as archived, create a separate child row, and move the
current-conversation binding to that child. Use this only with an adapter that
supports `placement: "current"`. Core owns the archive, create, rollback, and
session hook semantics; the channel adapter continues to own only the durable
