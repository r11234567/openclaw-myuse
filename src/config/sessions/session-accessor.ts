...
export async function commitReplySessionInitialization(params: {
  ...
  const committed = await updateSessionStore(
    ...
    async (store): Promise<ReplySessionInitializationCommitResult> => {
      ...
      const mergedSessionEntry = currentEntry
      ...
      if (params.preservePreviousSessionRow && currentEntry) {
        ...
        const archivedEntry = { ...(params.previousEntry ?? currentEntry) };
        archivedEntry.archivedAt ??= Date.now();
        delete archivedEntry.pinnedAt;
        store[resolved.normalizedKey] = archivedEntry;
        store[activeResolved.normalizedKey] = mergedSessionEntry;
        committedSessionKey = activeResolved.normalizedKey;
      } else {
        store[resolved.normalizedKey] = mergedSessionEntry;
      }
      ...
      return {
        ...
        sessionEntry: { ...(store[committedSessionKey] ?? sessionEntry) },
  ...
  const previousSessionTranscript = params.preservePreviousSessionRow
    ? {
...
export async function rollbackReplySessionInitialization(params: {
  ...
  return await updateSessionStore(
    ...
    (store) => {
      ...
      if (active.existing?.sessionId !== params.activeSessionId) {
        return false;
      }
      const source = resolveSessionStoreEntry({ store, sessionKey: params.sessionKey });
      if (source.existing?.sessionId !== params.previousEntry.sessionId) {
        return false;
      }
      store[source.normalizedKey] = { ...params.previousEntry };
      delete store[active.normalizedKey];
      for (const legacyKey of active.legacyKeys) {
        delete store[legacyKey];
      }
      return true;
    },
    {
      reentrant: true,
      skipSaveWhenResult: (rolledBack) => !rolledBack,
    },
  );
}
