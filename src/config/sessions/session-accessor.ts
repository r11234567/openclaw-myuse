...
export async function commitReplySessionInitialization(params: {
  ...
  previousEntry?: SessionEntry;
  /** Keep the prior row/transcript and write the initialized entry under activeSessionKey. */
  preservePreviousSessionRow?: boolean;
  ...
  let archivedSessionEntry: SessionEntry | undefined;
  let committedSessionKey = resolved.normalizedKey;
  const preservePreviousSessionRow =
    params.preservePreviousSessionRow === true && currentEntry !== undefined;
  const upserts: SessionEntryLifecycleUpsert[] = [
    {
      ...
      buildEntry: ({ store: currentStore }) => {
        ...
        committedSessionEntry = commitEntry
          ...
          : sessionEntry;
        if (preservePreviousSessionRow && commitEntry) {
          const activeResolved = resolveSessionStoreEntry({
            store: currentStore,
            sessionKey: params.activeSessionKey,
          });
          if (activeResolved.existing) {
            throw new Error(`replacement session key already exists: ${params.activeSessionKey}`);
          }
          committedSessionKey = activeResolved.normalizedKey;
          archivedSessionEntry = { ...commitEntry };
          archivedSessionEntry.archivedAt ??= Date.now();
          delete archivedSessionEntry.pinnedAt;
          return archivedSessionEntry;
        }
        return committedSessionEntry;
      },
    },
  ];
  if (preservePreviousSessionRow) {
    upserts.push({
      sessionKey: params.activeSessionKey,
      buildEntry: ({ currentEntry: activeEntry }) => {
        if (staleCommit) {
          return null;
        }
        if (activeEntry) {
  ...
  store[resolved.normalizedKey] = archivedSessionEntry ?? committedSessionEntry;
  if (preservePreviousSessionRow) {
    store[committedSessionKey] = committedSessionEntry;
  }
  if (params.retiredEntry) {
  ...
  const previousSessionTranscript = preservePreviousSessionRow
    ? {
...
/** Restores the source row when rebinding a newly initialized session fails. */
export async function rollbackReplySessionInitialization(params: {
  ...
  const active = resolveSessionStoreEntry({ store, sessionKey: params.activeSessionKey });
  const source = resolveSessionStoreEntry({ store, sessionKey: params.sessionKey });
  if (
    active.existing?.sessionId !== params.activeSessionId ||
    source.existing?.sessionId !== params.previousEntry.sessionId
  ) {
    return false;
  }
  ...
  try {
    await applySessionEntryLifecycleMutation({
      activeSessionKey: source.normalizedKey,
      removals: [
        {
          sessionKey: active.normalizedKey,
          expectedEntry: { ...active.existing },
        },
      ],
      storePath: params.storePath,
      upserts: [
        {
          sessionKey: source.normalizedKey,
          buildEntry: ({ currentEntry, store: currentStore }) => {
            if (
              currentStore[active.normalizedKey] ||
              currentEntry?.sessionId !== params.previousEntry.sessionId
            ) {
              throw new Error("reply session rollback state changed");
            }
            rolledBack = true;
